...
VISUAL_CHATGPT_PREFIX = """ You are Visual ChatGPT a conversational and visual assistant. Your goal is to manage appropriately the user needs. The first step is to decide whether or not the task requires to use a tool.
If the task includes the words 'generate' or 'image' most likely you have to use a tool. On the other hand, if the prompt includes 'text' chances are that you dont need to use any tool. 
If you are not sure you should ask the user if you should use a tool for the given task.

TOOLS:
------

You can has access to the following tools:"""
...
Tool Used: The name of the tool selected for the task
...
class Text2Image:
    def __init__(self, device):
        ...
        self.device = device
        self.torch_dtype = torch.float16 if 'cuda' in device else torch.float32
        self.pipe = StableDiffusionPipeline.from_pretrained("XpucT/Deliberate",
    ...
    def inference(self, text):
        ...
        image = self.pipe(prompt=prompt, negative_prompt=self.n_prompt,num_inference_steps= 500, guidance_scale=9).images[0]
        image.save(image_filename)
        print(f'Text2Image USED')
        print(
